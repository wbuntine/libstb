/* randist/gamma.c + randist/gausszig.c
 *
 * This version combines and modifies the two files for stand-alone
 * use outside GSL.  By Wray Buntine 2013.  Please note copyright and GPL.
 *
...
gsl_rng_uniform_int (unsigned long int n)
{
  unsigned long int range = 1U<<30U;
...
double
gsl_rng_gaussian_ziggurat (const double sigma)
{
  ...
  while (1)
    {
      i = gsl_rng_uniform_int (256); /*  choose the step */
      j = gsl_rng_uniform_int (16777216);  /* sample from 2^24 */
      sign = (i & 0x80) ? +1 : -1;
      i &= 0x7f;
    ...
    while (1)
      {
        do
          {
            x = gsl_rng_gaussian_ziggurat (1.0);
            v = 1.0 + c * x;
          }
        while (v <= 0);
...
double
gsl_rng_beta (const double a, const double b)
{
  double x1 = gsl_rng_gamma (a);
  double x2 = gsl_rng_gamma (b);
